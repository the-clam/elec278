...
int modified_quicksort(int *a, int left, int right)
{
	while(left < right)
	{
		//If the number of elements to be sorted in the current iteration is less than 32, then use insertion sort.
		if((right+1)-left < 32)
		{
			insertionsort(a, left, right);
			break;
		}
		else // Otherwise, use quicksort as before.
		{
			int  pivotndx = partition (a, left, right);
			if(right-pivotndx > pivotndx-left)
			{
				modified_quicksort(a, pivotndx+1, right);
				right = pivotndx-1;
			}
			else
			{	
				modified_quicksort(a, left, pivotndx-1);
				left = pivotndx+1;
			}
		}	
	}
